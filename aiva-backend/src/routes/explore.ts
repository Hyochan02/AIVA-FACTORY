...
// ── 공통: duration 컬럼 ───────────────────────────────────────
// v3부터 버전(variation)마다 별도의 tracks row이고, 생성 완료 시
// applySunoSuccess()가 Suno 실제 출력 길이를 t.duration에 직접 저장하므로
// 더 이상 track_versions를 조인할 필요가 없다.
const DURATION_SUBQ = `t.duration AS duration`
