...
const Login: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    // TODO: 2차 - 실제 API 연동 (POST /api/auth/login)
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="flex flex-col gap-2 mb-6">
            {[
              ...
              { icon: '🔵', label: 'Facebook으로 계속하기', bg: 'bg-[#1877F2]', text: 'text-white' },
            ].map(s => (
              <button key={s.label} className={`${s.bg} ${s.text} w-full py-2.5 px-4 rounded-[12px] text-sm font-semibold flex items-center justify-center gap-3 border border-(--border-color) hover:opacity-90 transition-opacity`}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="flex justify-end mt-1.5">
                <a href="#" className="text-xs text-indigo-400 hover:text-indigo-300">비밀번호 찾기</a>
              </div>
            </div>
