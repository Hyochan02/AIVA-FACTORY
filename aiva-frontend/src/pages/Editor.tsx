...
const Editor: React.FC = () => {
  ...
  } = useApi<{ jobs: JobHistory[] }>(
    () => getJobHistory(activeTab),
    [activeTab],
  );
  ...
  const [extendResult, setExtendResult] = useState<{ audioUrl: string } | null>(
    null,
  );
  ...
  // ── separate ──
  const [separateType, setSeparateType] = useState<
    "separate_vocal" | "split_stem"
  >("separate_vocal");
  const [separateResult, setSeparateResult] = useState<SeparateResult | null>(
    null,
  );
  ...
  const handleAction = async () => {
    ...
    try {
      if (activeTab === "extend") {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const res = (await convertWav({
          trackId: selectedTrackId,
          versionNum: wavVersion,
        })) as any;
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
            ...
            <div className="flex items-center gap-3 mb-1">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  {tabInfo.desc} · 크레딧 {tabInfo.credit}개 소모
                </p>
              </div>
            </div>
          </div>
              ...
              {(
                [
                  ...
                  },
                  {
                    ...
                    label: "전체 악기 분리",
                    credit: "50크레딧",
                    desc: "최대 12개 파일: 보컬, 드럼, 베이스, 기타, 키보드 등",
                  },
                ] as const
              ).map((opt) => (
                <button
                  key={opt.value}
                  type="button"
                  onClick={() => setSeparateType(opt.value)}
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center ${separateType === opt.value ? "border-indigo-400" : "border-slate-600"}`}
                  >
                    {separateType === opt.value && (
          ...
          {activeTab === "video" && (
            <div className="bg-[#0d1340] border border-primary-soft rounded-2xl p-5">
              <p className="text-sm text-slate-400">
                선택한 트랙으로 비주얼 이펙트가 포함된 MP4 뮤직비디오를
                생성합니다.
              </p>
              <p className="text-xs text-slate-500 mt-2">
                생성된 비디오는 15일간 보관됩니다.
          ...
          {successMsg && (
            <div className="bg-green-900/30 border border-green-700/40 rounded-xl p-4 text-sm text-green-300">
              {successMsg}
            </div>
          )}
          ...
          <Button
            ...
            disabled={
              loading ||
              isPolling ||
              (activeTab !== "lyrics" && !selectedTrackId)
            }
          >
            {isPolling ? (
              ...
              <div className="space-y-2">
                {Array(4)
                  ...
                  .map((_, i) => (
                    <div
                      key={i}
                      className="h-14 bg-navy-800/40 rounded-xl animate-pulse"
                    />
                  ))}
              </div>
            )}
            ...
            {!historyLoading && jobHistory.length === 0 && (
              <div className="flex flex-col items-center gap-1.5 py-8 text-slate-600">
                ...
                <p className="text-xs text-center">
                  {TAB_INFO.find((t) => t.id === activeTab)?.label} 기록이
                  없습니다
                </p>
              </div>
            )}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span
                                className={`w-1.5 h-1.5 rounded-full shrink-0 ${STATUS_DOT[job.status] ?? "bg-slate-400"}`}
                              />
        ...
        <div className="bg-[#0d1340] border border-primary-soft rounded-2xl p-6">
          ...
          {activeTab === "lyrics" && lyricsResult?.status === "done" && (
            <div className="space-y-3">
              ...
              <button
                onClick={() =>
                  navigator.clipboard.writeText(lyricsResult.text ?? "")
                }
                className="inline-flex items-center gap-1.5 text-xs text-indigo-400 hover:text-indigo-300"
              >
                <svg
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                .map((track) => (
                  <div
                    key={track.label}
                    className="bg-navy-800/30 rounded-xl p-3 space-y-2"
                  >
